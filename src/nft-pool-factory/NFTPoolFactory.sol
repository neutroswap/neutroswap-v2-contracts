...
contract NFTPoolFactory {
  INeutroMaster public immutable master; // Address of the master
  ...
  constructor(
    address master_,
    address neutroToken_,
    address xNeutroToken_
  ) {
    master = INeutroMaster(master_);
    neutroToken = IERC20(neutroToken_);
  ...
  function createPool(address lpToken) external returns (address pool) {
    ...
    NFTPool(pool).initialize(master, neutroToken, xNeutroToken, IERC20(lpToken));
