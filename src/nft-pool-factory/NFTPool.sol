 ...
 * spNFTs are yield-generating positions when the NFTPool contract has allocations from the Neutro Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Neutro staking position NFT", "spNFT") {
  ...
  INeutroMaster public master; // Address of the master
  ...
  function initialize(INeutroMaster master_, IERC20 neutroToken, IXNeutroToken xNeutroToken, IERC20 lpToken) external {
  ...
  event MergePositions(address indexed user, uint256[] tokenIds, uint256 destLockDuration);
  ...
  function mergePositions(uint256[] calldata tokenIds, uint256 lockDuration) external nonReentrant {
    ...
    emit MergePositions(msg.sender, tokenIds, lockDuration);
  }
