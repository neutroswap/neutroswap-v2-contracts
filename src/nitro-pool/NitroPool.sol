...
contract NitroPool is ReentrancyGuard, Ownable, INFTHandler {
    ...
    using SafeERC20 for IXNeutroToken;
    ...
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 neutroAmount, uint256 xNeutroAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        ...
        // if not whitelisted, the NitroPool can't transfer any xNEUTRO rewards
        require(to != address(this) || xNeutroToken.isTransferWhitelisted(address(this)), "cant handle rewards");
