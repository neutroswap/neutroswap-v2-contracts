...
contract NitroPoolFactory is Ownable, INitroPoolFactory {
  ...
  INeutroToken public neutroToken; // NEUTROToken contract's address
  IXNeutroToken public xNeutroToken; // xNEUTROToken contract's address
  ...
  constructor(address neutroToken_, address xNeutroToken_, address emergencyRecoveryAddress_, address feeAddress_){
  ...
  function createNitroPool(
    ...
    nitroPool = address(
      new NitroPool(
        address(neutroToken), address(xNeutroToken), msg.sender, nftPoolAddress,
