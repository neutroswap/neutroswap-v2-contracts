...
/*
 * This contract is used to distribute dividends to users that allocated xNeutro here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xNeutro allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IXNeutroTokenUsage, IDividends {
  ...
  address public immutable xNeutroToken; // xNeutroToken contract

  mapping(address => uint256) public usersAllocation; // User's xNeutro allocation
  ...
  constructor(address xNeutroToken_, uint256 startTime_) {
    require(xNeutroToken_ != address(0), "zero address");
  ...
  modifier xNeutroTokenOnly() {
    require(msg.sender == xNeutroToken, "xNeutroTokenOnly: caller should be XNeutroToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xNeutro to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xNeutro allocation from this dividends contract
   *
   * Can only be called by xNeutroToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxNeutroTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xNeutro is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userXNeutroAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userXNeutroAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userXNeutroAllocation.mul(accDividendsPerShare).div(1e18);
