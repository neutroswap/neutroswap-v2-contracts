...
/*
 * This contract is a xNEUTRO Usage (plugin) that can boost spNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the xNEUTROToken contract
 */
contract YieldBooster is Ownable, ReentrancyGuard, IXNeutroTokenUsage, IYieldBooster {
  ...
  // User's position xNEUTRO total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's xNEUTRO total allocation
  ...
  /**
   * @dev Returns total xNEUTRO allocated to this contract by "poolAddress"
   */
  ...
  /**
   * @dev Returns allocated xNEUTRO to "tokenId" spNFT from "poolAddress" NFTPool by "userAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of xNEUTRO to this usage contract
   ...
   */
  function allocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xNeutroTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _allocate(userAddress, poolAddress, tokenId, amount);

    // allocated xNEUTRO is added (as boost points) to spNFT
    INFTPool(poolAddress).boost(tokenId, amount);
  }

  /**
   * Deallocates "userAddress" user's "amount" of xNEUTRO from this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by XNeutroToken contract, which is trusted to verify amounts
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xNeutroTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _deallocate(userAddress, poolAddress, tokenId, amount);

    // should only be called if spNFT has not been burned, to avoid having stuck xNEUTRO on it
    if(INFTPool(poolAddress).exists(tokenId)) {
      // allocated xNEUTRO is removed (as boost points) from the spNFT
   ...
   * The pool should remove the allocated xNEUTRO (boost points) from its own spNFT when calling this function
   */
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's xNEUTRO allocations balance
    xNeutroToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of xNEUTRO from XNeutroToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = xNeutroToken.usageAllocations(msg.sender, address(this));

    // update user's xNEUTRO allocations balance
    xNeutroToken.deallocateFromUsage(msg.sender, amount);
  }
  ...
  /**
   * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated xNEUTRO)
   *
   ...
   * 1% of the pool's allocated xNEUTRO
   *
  ...
  /**
   * @dev Allocates "userAddress" user's "amount" of xNEUTRO to "tokenId" spNFT of "poolAddress" pool
   */
  ...
  /**
   * @dev Deallocates "userAddress" user's "amount" of xNEUTRO allocated to "tokenId" spNFT of "poolAddress" pool
   */
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated xNeutro");
