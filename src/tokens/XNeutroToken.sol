...
/*
 * xNEUTRO is Neutro's escrowed governance token obtainable by converting NEUTRO to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to NEUTRO through a vesting process
 * This contract is made to receive xNEUTRO deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract XNeutroToken is Ownable, ReentrancyGuard, ERC20("Neutro escrowed token", "xNEUTRO"), IXNeutroToken {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeERC20 for INeutroToken;

  struct XNeutroBalance {
    uint256 allocatedAmount; // Amount of xNEUTRO allocated to a Usage
    uint256 redeemingAmount; // Total amount of xNEUTRO currently being redeemed
  }

  struct RedeemInfo {
    uint256 neutroAmount; // NEUTRO amount to receive when vesting has ended
    uint256 xNeutroAmount; // xNEUTRO amount to redeem
    uint256 endTime;
    IXNeutroTokenUsage dividendsAddress;
    uint256 dividendsAllocation; // Share of redeeming xNEUTRO to allocate to the Dividends Usage contract
  }

  INeutroToken public immutable neutroToken; // NEUTRO token to convert to/from
  IXNeutroTokenUsage public dividendsAddress; // Neutro dividends contract

  EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive xNEUTRO

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate xNEUTRO
  mapping(address => mapping(address => uint256)) public override usageAllocations; // Active xNEUTRO allocations to usages

  uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating xNEUTRO
  ...
  // Adjusted dividends rewards for redeeming xNEUTRO
  uint256 public redeemDividendsAdjustment = 50; // 50%

  mapping(address => XNeutroBalance) public xNeutroBalances; // User's xNEUTRO balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances

  constructor(address neutroToken_) {
  ...
  event Redeem(address indexed userAddress, uint256 xNeutroAmount, uint256 neutroAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 xNeutroAmount, uint256 neutroAmount);
  event CancelRedeem(address indexed userAddress, uint256 xNeutroAmount);
  ...
  /*
   * @dev Returns user's xNEUTRO balances
   */
  function getXNeutroBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    XNeutroBalance storage balance = xNeutroBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable NEUTRO for "amount" of xNEUTRO vested for "duration" seconds
   */
  function getNeutroByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 neutroAmount, uint256 xNeutroAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.neutroAmount, _redeem.xNeutroAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }

  /**
   * @dev returns approved xNeutro to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated xNeutro from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns if "account" is allowed to send/receive xNEUTRO
   */
  ...
  function updateDividendsAddress(address dividendsAddress_) external onlyOwner {
    ...
    dividendsAddress = IXNeutroTokenUsage(dividendsAddress_);
  }
  ...
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    require(account != address(this), "updateTransferWhitelist: Cannot remove xNeutro from whitelist");
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of xNEUTRO from msg.sender
   */
  function approveUsage(IXNeutroTokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Convert caller's "amount" of NEUTRO to xNEUTRO
   */
  ...
  /**
   * @dev Convert caller's "amount" of NEUTRO to xNEUTRO to "to" address
   */
  ...
  /**
   * @dev Initiates redeem process (xNEUTRO to NEUTRO)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 xNeutroAmount, uint256 duration) external nonReentrant {
    require(xNeutroAmount > 0, "redeem: xNeutroAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), xNeutroAmount);
    XNeutroBalance storage balance = xNeutroBalances[msg.sender];

    // get corresponding NEUTRO amount
    uint256 neutroAmount = getNeutroByVestingDuration(xNeutroAmount, duration);
    emit Redeem(msg.sender, xNeutroAmount, neutroAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(xNeutroAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = xNeutroAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(neutroAmount, xNeutroAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
    } else {
      // immediately redeem for NEUTRO
      _finalizeRedeem(msg.sender, xNeutroAmount, neutroAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.xNeutroAmount);
    _finalizeRedeem(msg.sender, _redeem.xNeutroAmount, _redeem.neutroAmount);

    // handle dividends compensation if any was active
    if(_redeem.dividendsAllocation > 0) {
      // deallocate from dividends
      IXNeutroTokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
    }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming xNEUTRO available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.xNeutroAmount);
    _transfer(address(this), msg.sender, _redeem.xNeutroAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.xNeutroAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available xNEUTRO to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates xNEUTRO to usageContract
    IXNeutroTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available xNEUTRO from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required xNeutro xNEUTRO from "userAddress"
   */
  ...
  /**
   * @dev Deallocates caller's "amount" of available xNEUTRO from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate xNEUTRO into usageContract
    IXNeutroTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated xNEUTRO belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate xNEUTRO from itself
   */
  ...
  /**
   * @dev Convert caller's "amount" of NEUTRO into xNEUTRO to "to"
   */
  function _convert(uint256 amount, address to) internal {
    ...
    neutroToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "neutroAmount" and removing "xNeutroAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * NEUTRO excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 xNeutroAmount, uint256 neutroAmount) internal {
    uint256 neutroExcess = xNeutroAmount.sub(neutroAmount);

    // sends due NEUTRO tokens
    neutroToken.safeTransfer(userAddress, neutroAmount);

    // burns NEUTRO excess if any
    neutroToken.burn(neutroExcess);
    _burn(address(this), xNeutroAmount);

    emit FinalizeRedeem(userAddress, xNeutroAmount, neutroAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available xNEUTRO to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedXNeutro = usageApprovals[userAddress][usageAddress];
    require(approvedXNeutro >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedXNeutro.sub(amount);
    ...
    // adjust user's xNEUTRO balances
  ...
  /**
   * @dev Deallocates "amount" of available xNEUTRO to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated xNEUTRO to this usage to deallocate
    ...
    // burn corresponding NEUTRO and XNEUTRO
    neutroToken.burn(deallocationFeeAmount);
