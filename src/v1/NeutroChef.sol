...
import "./interfaces/IMultipleRewards.sol";
import "./interfaces/INeutroPair.sol";
import "./libraries/BoringERC20.sol";

contract NeutroChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Neutro to distribute per block.
        uint256 lastRewardTimestamp; // Last block number that Neutro distribution occurs.
        uint256 accNeutroPerShare; // Accumulated Neutro per share, times 1e18. See below.
    ...
    IBoringERC20 public neutro;

    // Neutro tokens created per second
    uint256 public neutroPerSec;
    ...
    // The timestamp when Neutro mining starts.
    uint256 public startTimestamp;

    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Total Neutro in Neutro Pools (can be multiple pools)
    uint256 public totalNeutroInPools;
    ...
    constructor(
        IBoringERC20 _neutro,
        uint256 _neutroPerSec,
    ...
    // Update the given pool's Neutro allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
        ...
        )
    {
        ...
        uint256 accNeutroPerShare = pool.accNeutroPerShare;
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 neutroReward = (multiplier *
                neutroPerSec *
                pool.allocPoint *
                lpPercent) /
                totalAllocPoint /
            ...
            accNeutroPerShare += (
                ((neutroReward * ACC_TOKEN_PRECISION) / lpSupply)
            );
        }

        uint256 pendingNeutro = (((user.amount * accNeutroPerShare) /
        ...
        addresses[0] = address(neutro);
        symbols[0] = IBoringERC20(neutro).safeSymbol();
        decimals[0] = IBoringERC20(neutro).safeDecimals();
        amounts[0] = pendingNeutro;
        ...
        )
    {
        ...
        rewardsPerSec[0] =
            (pool.allocPoint * neutroPerSec * lpPercent) /
            totalAllocPoint /
            total;
    ...
    // View function to see if user can harvest Neutro.
    function canHarvest(
        uint256 _pid,
        address _user
    ...
    function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
        ...
        uint256 neutroReward = ((multiplier * neutroPerSec) * pool.allocPoint) /
            totalAllocPoint;
        ...
        if (marketingPercent > 0) {
            neutro.mint(
                marketingAddress,
                (neutroReward * marketingPercent) / total
            );
        }

        if (teamPercent > 0) {
            neutro.mint(teamAddress, (neutroReward * teamPercent) / total);
        }

        neutro.mint(address(this), (neutroReward * lpPercent) / total);

        pool.accNeutroPerShare +=
            (neutroReward * ACC_TOKEN_PRECISION * lpPercent) /
            pool.totalLp /
            total;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        INeutroPair pair = INeutroPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount);
    }

    // Deposit tokens for Neutro allocation.
    ...
    function _deposit(
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(neutro)) {
                totalNeutroInPools += _amount;
            }
        }
        user.rewardDebt =
            (user.amount * pool.accNeutroPerShare) /
            ACC_TOKEN_PRECISION;
        ...
        for (
            ...
            pool.rewarders[rewarderId].onNeutroReward(
                _pid,
                msg.sender,
                user.amount
            );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        for (
            ...
            pool.rewarders[rewarderId].onNeutroReward(_pid, msg.sender, 0);
        }

        if (address(pool.lpToken) == address(neutro)) {
            totalNeutroInPools -= amount;
        }
    ...
    // Pay or lockup pending Neutro.
    function payOrLockupPendingNeutro(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = ((user.amount * pool.accNeutroPerShare) /
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.rewardLockedUp = 0;
                user.nextHarvestUntil = block.timestamp + pool.harvestInterval;
                safeNeutroTransfer(msg.sender, pendingRewards);
            }
        } else if (pending > 0) {
    ...
    function safeNeutroTransfer(address _to, uint256 _amount) internal {
        if (neutro.balanceOf(address(this)) > totalNeutroInPools) {
            uint256 neutroBal = neutro.balanceOf(address(this)) -
                totalNeutroInPools;
            if (_amount >= neutroBal) {
                neutro.safeTransfer(_to, neutroBal);
            } else if (_amount > 0) {
                neutro.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _neutroPerSec) public onlyOwner {
        _massUpdatePools();

        emit EmissionRateUpdated(msg.sender, neutroPerSec, _neutroPerSec);
    ...
    function getNeutroPerSec() public view returns (uint256) {
        return neutroPerSec;
    }
}
